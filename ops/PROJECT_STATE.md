...
- **2026-09-10 四批优化**（任务1-4 串行完成）：
  ...
  - **unknown 人工标注归类**（2026-09-10）：`regs --unknown --report X` 从报告聚合未映射 title（实时 _lookup 判断，标注立即生效）；`regs --annotate "TITLE" "REGULATION" [ARTICLE] --note N` 写 `~/.hermes/agentlens-reports/annotations.json`（env AGENTLENS_ANNOTATIONS 覆盖）；annotations **优先于**静态表（人工判定覆盖自动化）；`regs --annotations` 列出全部标注。mcp_server.py 双版本兼容（mcp SDK 1.x FastMCP / 2.x MCPServer try-import）
