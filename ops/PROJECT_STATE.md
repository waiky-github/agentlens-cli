...
- 测试：tests/ pytest **222 用例全绿**（公共 venv /home/agentuser/.hermes/hermes-agent/venv/bin/python -m pytest tests/ -q）
- **2026-09-10 四批优化**（任务1-4 串行完成）：
  ...
  - **unknown 人工标注归类**（2026-09-10）：`regs --unknown --report X` 从报告聚合未映射 title（实时 _lookup 判断，标注立即生效）；`regs --annotate "TITLE" "REGULATION" [ARTICLE] --note N` 写 annotations.json（env AGENTLENS_ANNOTATIONS 覆盖）；annotations **优先于**静态表（人工判定覆盖自动化）；`regs --annotations` 列出全部标注。mcp_server.py 双版本兼容（mcp SDK 1.x FastMCP / 2.x MCPServer try-import）。**路径分叉修复**：annotations 默认路径绑定 AGENTLENS_REPORT_DIR（与其他模块统一，修复 $HOME 重定向下 CLI 与会话写不同文件的 bug）
